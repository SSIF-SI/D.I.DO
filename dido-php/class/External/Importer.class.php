...
class Importer {
	...
	public function __construct($FTPConnector = null, $connInstance = null) {
		if (is_null ( $FTPConnector ))
			$FTPConnector = FTPConnector::getInstance ();
		if (is_null ( $connInstance ))
			$connInstance = DBConnector::getInstance ();
	...
	public function import($data) {
		...
		if (! isset ( $data ['import_filename'] ) || ! isset ( $data ['md_nome'] ) || ! isset ( $data ['md_type'] ) || ! isset ( $data ['md_xml'] ))
			die ( json_encode ( array (
					'errors' => 'Mancano argomenti' 
			) ) );
			
			// Se non trovo il file vuol dire che è stato importato
			// correttamente da qualcun'altro
		if (! file_exists ( GECO_IMPORT_PATH . $data ['import_filename'] ))
			die ( json_encode ( array (
					'errors' => "Il file è già stato importato" 
			) ) );
		...
		if (! $rename)
			die ( json_encode ( array (
					'errors' => 'Permessi di scrittura negati' 
			) ) );
	...
	// Rinomino e Copio il file sample06.pdf nella directory FTP dove dovrà
	// essere trasferito il file importato
