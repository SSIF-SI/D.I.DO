...
class master_log extends Crud {
	...
	public function updateMasterLog($operation = null, $tables = null, $esito = "OK") {
		if (is_null ( $this->_idFlusso )) {
			if (is_null ( $operation ) || is_null ( $tables ))
				throw new Exception ( __CLASS__ . __METHOD__ . ': Missing $operation and $tables parameters' );
			$this->_connInstance->query ( "INSERT INTO 
						{$this->TABLE} (codice_flusso, tabelle, operazione, data_inizio_oper) 
						 VALUES 
							('" . self::CODICE_FLUSSO . "', '$tables', '$operation', now())", self::SEQ_ID_FLUSSO );
			$this->_idFlusso = $this->_connInstance->getLastInsertId ();
		} else {
			$this->_connInstance->query ( "UPDATE 
						{$this->TABLE}
					 SET
						data_fine_oper = now(),
						esito_oper = '$esito'
	...
	public function getLastIdFlussoOk($table) {
		$this->_connInstance->query ( "SELECT 
						id_flusso
					 FROM
						{$this->TABLE}
					 WHERE
