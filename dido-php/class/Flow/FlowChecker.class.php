...
class FlowChecker extends ClassWithDependencies {
	...
	public function checkMasterDocument(array $id) {
		...
		if ($md) {
			...
			// $fileList = array("ordine di
			// missione.pdf","allegato_1.pdf","allegato_2.pdf");
			...
			foreach ( $this->_XMLParser->getDocList () as $document ) {
				...
				if (! is_null ( $document ['md'] )) {
					...
					// $files = self::_getFtpFiles($docResult->documentName,
					// $fileList);
	...
	public static function getDocInputsByIdDoc($result, $id_doc) {
		if (isset ( $result ['data'] ['info'] ['documents'] [$id_doc] )) {
			...
			if (empty ( $document->inputs ))
				return $document->defaultInputs;
			return array (
					$document->inputs,
					$document->defaultInputs 
			);
		} else
			return array ();
	}
	...
	private function _checkSignatures($filename, $document, $signers, $k, &$docResult, $md_data) {
		...
		foreach ( $document->signatures->signature as $signature ) {
			...
			$pKeys = array (
					$signers [$who] ['id_persona'] => $signers [$who] ['pkey'],
					$signers [$who] ['id_delegato'] => $signers [$who] ['pkey_delegato'] 
			);
