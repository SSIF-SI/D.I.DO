...
class PermissionHelper {
	...
	private function __construct() {
		...
		$signer = array_merge ( Utils::filterList ( $signatures, "id_persona", $user_id ), Utils::filterList ( $signatures, "id_delegato", $user_id ) );
	...
	public function getUserField($field) {
		if (! isset ( $this->_user [$field] ))
			Throw new Exception ( __CLASS__ . ":" . __METHOD__ . " User has no field $field" );
	...
	public function isGestore($service = null) {
		...
		if ($this->_role == self::RUOLO_AMMINISTRATORE)
			return true;
	...
	public function isConsultatore($service = null) {
		if (is_null ( $service ))
			return $this->_role == self::RUOLO_CONSULTATORE;
		
		if ($this->_role == self::RUOLO_AMMINISTRATORE || $this->_role == self::RUOLO_GESTORE)
			return true;
