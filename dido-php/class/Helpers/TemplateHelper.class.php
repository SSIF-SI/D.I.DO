		...
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<div class="row">
					...
					<a class="btn btn-primary edit-info"
						href="?md=<?=$_GET['md']?>&edit"> <span
						class="fa fa-pencil fa-1x fa-fw"></span> Modifica informazioni
					</a>
				</div>
	                               	<?php endif;?>
	                               	<hr />
		...
		foreach ( $flowCheckereResult ['doclist'] as $docName => $docData ) :
			...
			if (empty ( $docData->found )) :
				...
				foreach ( $docData->found as $id_doc => $found ) :
					...
					$status = empty ( $docData->errors ) ? ($docData->mandatory ? 'success' : 'not-mandatory') : (isset ( $docData->errors ['missing'] ) ? 'missing-document' : 'missing-signatures');
		...
		<div class="row">
			...
			<div class="col-lg-8">
					...
					class="text-right">
		                                                        	...
		                                                        	<a
						class="btn btn-info upload-pdf" type="button"><span
						class="fa fa-upload fa-1x fa-fw"></span> <?= isset($docData->errors['missing']) ? "Carica" : "Aggiorna"?> il Pdf</a>
					                                            	<?php endif; ?>
					                                            	<?php if(!isset($docData->errors['missing']) && $actions['canDownload']):?>
					                                               	<a
						class="btn btn-info download-pdf" type="button"><span
						class="fa fa-download fa-1x fa-fw"></span> Scarica il Pdf</a>
					                                               	<?php endif;?>
					                                            </form>
					                                            <?php else:?>
					                                            <div
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
																<?php if(!isset($docData->errors['missing'])): $docData->found?>
			                        								...
			                        								<div class="row text-center">
							<a class="btn btn-primary edit-info"
								href="?md=<?=$_GET['md']?>&d=<?=$id_doc?>&edit"> <span
								class="fa fa-pencil fa-1x fa-fw"></span> Modifica informazioni
								documento
							</a>
						</div>
				...
				<div class="panel panel-info">
					...
					<div class="panel-body">
															<?php foreach($docData->signatures[$id_doc] as $signData):if($signData['result'] == 'skipped') continue;?>
				                                        		<div
							class="alert <?=$signData['result'] == false ? 'alert-danger' : 'alert-success'?>">
							<span
								class="fa fa-<?=$signData['result'] == false ? 'times' : 'check'?>"></span>&nbsp;<?=$signData['who']." ({$signData['role']})"?></div>
			...
			<div id="<?php echo "list".$category;?>"
				class="panel-collapse collapse">
					...
					foreach ( $val as $k => $data ) :
						$obj = unserialize ( file_get_contents ( "geco-import/" . $category . DIRECTORY_SEPARATOR . $data ['filename'] ) );
						...
						if (! $th) :
							...
							?>
							<tr>
						<th><input type="checkbox" class="select-all" rel="<?=$catName?>">&nbsp;Seleziona
							tutti</th>
						...
						<th><a class="btn btn-primary disabled action import-selected"
							href="#<?=$catName?>"> <span
								class="fa fa-sign-in fa-rotate-90 fa-1x fa-fw"></span> Importa
								Selezionati
						</a>&nbsp; <a
							class="btn btn-primary disabled action link-selected"
							href="#<?=$catName?>"> <span class="fa fa-link fa-1x fa-fw"></span>
								Collega
						</a></th>
					</tr>
						<?php endif;?>
						...
						<td>
							<div class="checkbox">
								<label> <input class="select-one" rel="<?=$catName?>"
						...
						foreach ( $inputs as $input ) :
							...
							if (isset ( $input ['shortView'] )) :
								...
								<?php endif; endforeach; ?>
							    <td>
							<form style="display: none" role="form" method="POST"
								id="importform-<?=$formId?>" enctype="multipart/form-data">
								<input type="hidden" id="import_filename" name="import_filename"
									value="<?=$category.DIRECTORY_SEPARATOR.$data['filename']?>" />
								<input type="hidden" id="md_xml" name="md_xml"
									value="<?=$xml['xml_filename']?>" /> <input type="hidden"
									id="md_nome" name="md_nome" value="<?=$data['md_nome']?>" /> <input
									type="hidden" id="md_type" name="md_type"
									value="<?=$data['type']?>" /> 
					                	<?php foreach($obj_new as $id=>$val_n):?>
					                	<input type="hidden"
									id="<?=FormHelper::fieldFromLabel($id)?>"
									name="<?=FormHelper::fieldFromLabel($id)?>" value="<?=$val_n?>" /> 
					                	<?php endforeach;?>
									</form> <a class="btn btn-primary import"
							href="#importform-<?=$formId?>"> <span
								class="fa fa-sign-in fa-rotate-90 fa-1x fa-fw"></span> Importa
						</a>
						</td>
					</tr>
				...
				class="panel-collapse collapse">
					...
					foreach ( $val as $id_md => $data ) :
						...
						foreach ( $inputs as $input ) :
							...
							href="?md=<?=$id_md?>"><span class="fa fa-search fa-1x fa-fw"></span>
								Dettaglio</a></td>
					</tr>
...
<?php	else :?>
<div class="alert alert-danger">Nessun documento aperto.</div>
