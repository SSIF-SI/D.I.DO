...
class MasterDocumentManager extends ClassWithDependencies{
	/*
	 * Questa classe si occuperà di fornire all'applicazione l'elenco completo dei Master document e relativi dati allegati
	 * che l'utente potrà visualizzare in base ai permessi che ha. 
	 * 
	 * Il formato di uscita sarà un array associativo come segue:
	 * 
	 * Array (
	 * 			[md] 				=> [id_md => record masterdocument]
	 * 			[md_data]			=> [id_md => [md_inputs]]
	 * 			[documents] 		=> [id_md => [id_doc => record document]]
	 * 			[documents_data]	=> [id_doc => [document_inputs]]
	 * )
	 */
	...
	public function getAllMyMasterDocuments(Array $filters = [], $categorized = true){
		...
		$mdToSearch = empty($filters) ? 
					  $this->_Masterdocument->getBy('closed', 0) : 
					  $this->_filterMD($this->_Masterdocument->getAll(),$filters);
		...
		$key_value = array();
		$ownerRules = (array) simplexml_load_file(FILES_PATH."ownerRules.xml")->inputField;
		
		foreach($ownerRules as $inputField){
			...
			foreach($mdToSearch as $id_md => $md){
				...
				if(count($docList)){
					foreach($docList as $id_doc=>$document){
						...
						$filename = $md['ftp_folder']. DIRECTORY_SEPARATOR. $md['nome']. "_". $id_md . DIRECTORY_SEPARATOR . 
							FormHelper::fieldFromLabel($document['nome']." ".$document['id_doc'].".".$document['extension']);
	...
	public function getSingleMasterDocument($id_md){
		...
		$this->getAllMyMasterDocuments(array(['field'=> 'id_md', 'value' => $id_md]), false);
