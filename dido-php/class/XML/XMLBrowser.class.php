...
class XMLBrowser {
	...
	public function getXmlTree($onlyOwner = false) {
		...
		foreach ( $filtered as $catName => $data ) {
			...
			if (empty ( $filtered [$catName] ['documenti'] ))
				unset ( $filtered [$catName] );
		}
				...
				foreach ( $versioni as $numVersione => $metadata ) {
					if (! is_null ( $services )) {
						if (! in_array ( ( string ) $metadata ['owner'], $services )) {
							...
							$isMDVisible = $this->_PermissionHelper->isGestore ( $metadata ['owner'] ) || $this->_PermissionHelper->isConsultatore ( $metadata ['owner'] );
	...
	public function getXmlFromNameAndData($name, $data) {
		foreach ( $this->_xmlTree as $categoria => $xmlData ) {
			foreach ( $xmlData ['documenti'] as $tipoDocumento => $versioni ) {
				...
				if ($tipoDocumento != $name)
					continue;
				foreach ( $versioni as $numVersione => $metadata ) {
					$xml = $metadata ['xml'];
					if (isset ( $xml ['validEnd'] ) && (string ( $xml ['validEnd'] )) < $data)
						continue;
					return array (
